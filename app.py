...
headings = ['itching', 'skin_rash', 'nodal_skin_eruptions', 'continuous_sneezing', 'shivering', 'chills', 'joint_pain', 'stomach_pain', 'acidity', 'ulcers_on_tongue',
...
checkbox_results = []
st.write("### Select the symptoms you are having:")
with st.empty():
    for i, heading in enumerate(headings):
        checkbox_results.append(st.checkbox(heading))

for i, heading in enumerate(headings):
    # Generate a unique key for each checkbox by appending the index to a base key
    checkbox_key = f"checkbox_{i}"
    checked = st.checkbox(heading, key=checkbox_key)
    if checked:
        symptoms[heading] = 1
